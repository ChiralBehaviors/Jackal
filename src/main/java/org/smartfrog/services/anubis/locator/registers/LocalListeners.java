...
public class LocalListeners {
	...
	public synchronized void checkNodes(View view) {
		...
		while (nodeIter.hasNext()) {
			...
			for (ProviderProxy proxy : providersByNode.getSet(node)) {
				...
				for (ProviderInstance instance = info.providers.remove(proxy); instance != null; instance = info.providers
						.remove(proxy)) {
	...
	public synchronized void deregister(AnubisListener listener) {
		...
		if (info.listeners.isEmpty()) {
			...
			Iterator<ProviderInstance> providerIter = info.providers.values()
					.iterator();
			...
			while (providerIter.hasNext()) {
				...
				if (providersByNode.remove(providerProxy.node, providerProxy)) {
					...
					if (!informedNodes.contains(providerProxy.node)) {
						RegisterMsg removeMsg = RegisterMsg
								.removeListener(info.proxy);
	...
	public synchronized String toString() {
		...
		for (Entry<Integer, Set<ProviderProxy>> entry : providersByNode
				.entrySet()) {
			str += "    " + entry.getKey();
