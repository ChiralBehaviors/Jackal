...
public class MessageNioHandler implements SendingListener, IOConnection,
	...
	public MessageNioHandler(Selector selector, SocketChannel sc,
			Vector<SelectionKey> deadKeys, Vector<SelectionKey> writePendingKeys, RxQueue rxQueue,
			WireSecurity sec) {
		...
		try {
			...
			if (log.isLoggable(Level.SEVERE)) {
				log.log(Level.SEVERE, me
						+ " failed to marshall timed message: " + tm
						+ " - not sent", ex);
			}
			return;
		}
	...
	public int writeData() {
		...
		int returnedInt = SENDING_REFUSED;
		long dataLeftToSend = (dataToWrite[0].remaining() + dataToWrite[1]
				.remaining());
