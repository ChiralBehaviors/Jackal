...
public class ConnectionSet implements ViewListener, HeartbeatReceiver {
	...
	private static final Logger log = Logger.getLogger(ConnectionSet.class
			.getCanonicalName()); // TODO should be Async wrapped
	...
	public synchronized MessageConnection connect(int id) {
		...
		MessageConnection mcon = new MessageConnection(identity, this,
				hbcon.getProtocol(), hbcon.getCandidate());
	...
	public synchronized void convertToHeartbeatConnection(MessageConnection mcon) {
		Connection con = new HeartbeatConnection(identity, this,
				mcon.getProtocol(), mcon.getCandidate());
	...
	public synchronized void convertToMessageConnection(HeartbeatConnection con) {
		...
		if (thisEndInitiatesConnectionsTo(con.getSender())) {
			MessageConnection mcon = new MessageConnection(identity, this,
					con.getProtocol(), con.getCandidate());
	...
	public void setHeartbeatCommsFactory(
	...
	public void setLeaderProtocolFactory(
	...
	public void deploy() throws IOException {
		...
		heartbeatComms = heartbeatCommsFactory.create(heartbeatAddress,
				connectionAddress, this, "Anubis: Heartbeat Comms (node "
						+ identity.id + ")", identity);

		SelfConnection self = new SelfConnection(identity, connectionView,
				connectionServer.getAddress(), isPreferredLeaderNode);
	...
	public synchronized String toString() {
		...
		str.append("\n").append(identity).append(" ConnectionSet view: ")
	...
	private boolean isBetterTimeStamp(long timeStamp) {
		return timeStamp != View.undefinedTimeStamp
				&& (connectionView.getTimeStamp() == View.undefinedTimeStamp || timeStamp < connectionView
						.getTimeStamp());
	}
}
