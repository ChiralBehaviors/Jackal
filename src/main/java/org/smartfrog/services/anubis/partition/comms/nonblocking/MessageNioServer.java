...
public class MessageNioServer extends Thread implements IOConnectionServer {
	...
	public MessageNioServer(ConnectionAddress address, Identity id,
		...
		try {
			...
			server.socket().bind(
					new InetSocketAddress(address.ipaddress, address.port));
		...
		pendingNewChannels = new Hashtable<SelectableChannel, MessageNioHandler>();
		...
		// new queue, RxQueue turns out to be a generic queue not specific to
		// rxJobs...
	...
	public String getThreadStatusString() {
		...
		if (connAdd != null) {
			buffer.append(" address = ")
					.append(connAdd.ipaddress.getHostName()).append(":")
					.append(connAdd.port);
		}
	...
	public void run() {
		try {
			...
			if (syncLog.isLoggable(Level.SEVERE)) {
				syncLog.log(Level.SEVERE, me.toString() + getName()
	...
	public void startConnection(ConnectionAddress conAd, Identity me,
		...
		try {
			...
			mnh = new MessageNioHandler(selector, sendingChannel, deadKeys,
					writePendingKeys, assignRxQueue(), wireSecurity);
		...
		while (open) {
			...
			synchronized (pendingNewChannels) {
				if (!pendingNewChannels.isEmpty()) {
					...
					MessageNioHandler mnh = null;
					for (Enumeration<SelectableChannel> en = pendingNewChannels
			...
			while (iter.hasNext()) {
				...
				if (key.isAcceptable()) {
					...
					try {
						...
						MessageNioHandler conHandler = new MessageNioHandler(
								selector, clientChannel, deadKeys,
								writePendingKeys, assignRxQueue(), wireSecurity);
						conHandler.init(me, connectionSet);
						// only register for read - write is still enabled by
						// default
				...
				if (key.isReadable()) {
					...
					MessageNioHandler mnh = (MessageNioHandler) key
							.attachment();
					ByteBuffer readFullBuffer = mnh.newDataToRead(key);
					if (readFullBuffer != null) {
						// create a container for the deserialized job and put
						// on decoupling queue
				...
				if (key.isConnectable()) {
					...
					// do here what was being done in MessageConnectionInitiator
					// before - send to a decoupling queue...
