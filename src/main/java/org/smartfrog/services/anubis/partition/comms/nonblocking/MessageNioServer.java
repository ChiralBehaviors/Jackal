...
public class MessageNioServer extends Thread implements IOConnectionServer {
    ...
    public MessageNioServer(ConnectionAddress address, Identity id, ConnectionSet cs, WireSecurity sec) throws Exception {
        ...
        try{
            ...
            if( debug && asyncLog.isLoggable(Level.FINER) )
                asyncLog.finer("MNS: server bound to: "+connAdd.ipaddress.getHostName()+" - "+connAdd.port);
    ...
    public void startConnection(ConnectionAddress conAd, Identity me, ConnectionSet cs, MessageConnection con, NonBlockingConnectionInitiator mci){
        if( debug && asyncLog.isLoggable(Level.FINER) )
            asyncLog.finer("MNS: startConnection is called: "+conAd.ipaddress.getHostName()+" - "+conAd.port);
    ...
    public void run() {
        try {
            ...
            if( syncLog.isLoggable(Level.SEVERE) )
                syncLog.log(Level.SEVERE, me.toString() + this.getName() + " THREAD HAS TERMINATED WITH THROWABLE", thr);
        }
    }
    ...
    private void runLoop(){
        ...
        while(open){
            if( debug && asyncLog.isLoggable(Level.FINER) )
                asyncLog.finer(this.getName()+": Calling select and presumably, block...");
            try{
            ...
            synchronized(pendingNewChannels){
                if(!pendingNewChannels.isEmpty()){
                    if( debug && asyncLog.isLoggable(Level.FINER) )
                        asyncLog.finer("MNS: Registering pending channels: "+pendingNewChannels.size());
            ...
            while(iter.hasNext()){
                if( debug && asyncLog.isLoggable(Level.FINER) )
                    asyncLog.finer(this.getName()+": going through the key iterator");
                key = (SelectionKey)iter.next();

                if (key.isAcceptable()){
                    if( debug && asyncLog.isLoggable(Level.FINER) )
                        asyncLog.finer(this.getName()+": isAcceptable() returned true");
                    try{
                ...
                if (key.isReadable()){
                    if( debug && asyncLog.isLoggable(Level.FINER) )
                        asyncLog.finer(this.getName()+": isReadable() returned true");
                ...
                if (key.isWritable()){
                    if( debug && asyncLog.isLoggable(Level.FINER) )
                        asyncLog.finer(this.getName()+": isWritable() returned true");
                ...
                if (key.isConnectable()){
                    if( debug && asyncLog.isLoggable(Level.FINER) )
                        asyncLog.finer(this.getName()+": isConnectable() returned true");
            ...
            synchronized(deadKeys){
                if(!deadKeys.isEmpty()){
                    if( debug && asyncLog.isLoggable(Level.FINER) )
                        asyncLog.finer("MNS: Killing of at least one key/channel: "+deadKeys.size());
            ...
            synchronized(writePendingKeys){
                ...
                if (!writePendingKeys.isEmpty()){
                    if( debug && asyncLog.isLoggable(Level.FINER) )
                        asyncLog.finer("MNS: Registering interest for write "+writePendingKeys.size());
                    SelectionKey pendingKey = null;
                    int pendingNumber = writePendingKeys.size();
                    if( debug && asyncLog.isLoggable(Level.FINER) )
                        asyncLog.finer("MNS: pending write NUmber: "+pendingNumber);
