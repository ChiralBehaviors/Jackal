    ...
    protected void deliverBytes(byte[] bytes) {
        ...
        try {
            ...
            if( log.isLoggable(Level.SEVERE) )
                log.severe(me + "multicast transport encountered security violation receiving message - ignoring the message " ); // + this.getSender() );
            return;
            
        }  catch (Exception ex) {
            if( log.isLoggable(Level.SEVERE) )
                log.log(Level.SEVERE, me + "Error reading wire form message - ignoring", ex);
            return;
        }
