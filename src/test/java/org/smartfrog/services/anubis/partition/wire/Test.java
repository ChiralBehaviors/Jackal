...
public class Test extends TestCase {
	...
	public void testIt3() {
		try {
			...
			ConnectionAddress outAddress = new ConnectionAddress(
					InetAddress.getLocalHost(), 1066);

			ConnectionAddress outTestIF = new ConnectionAddress(
					InetAddress.getLocalHost(), 2020);
			...
			System.out
					.println("Heartbeat Test message with getTestInterface is: "
							+ inMsg);
			inMsg.getSenderAddress();
			System.out
					.println("Heartbeat Test message with sender address is: "
							+ inMsg);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public void testIt() {
		try {
			...
			System.out
					.println("END HeartbeatTestMsg marshalling test run took "
							+ (end - start) + "ms");

			System.out
					.println("STARTING HeabeatTestMsg UNmarshalling test run");
			HeartbeatMsg inMsg;
			...
			System.out
					.println("END HeartbeatTestMsg UNmarshalling test run took "
							+ (end - start) + "ms");
